...
class RestoreInventoryOnRefund implements ShouldQueue
{
    ...
    public function handle(OrderRefunded $event): void
    {
        foreach ($event->order->lineItems as $item) {
            $ledger = $this->inventoryEngine->recordMovement(
                ...
                reference: "refund:{$event->order->orderNumber}",
                description: "Restocked from refund #{$event->order->orderNumber}",
            );
            ...
            $movement = new InventoryMovementDTO(
                ...
                type: 'return_restock',
                referenceType: 'order_refunded',
                referenceId: $event->order->orderId,
                reason: $event->reason,
                metadata: ['refund_amount' => $event->refundAmount],
    ...
    public function failed(OrderRefunded $event, \Throwable $e): void
    {
        logger()->error('RestoreInventoryOnRefund failed', [
