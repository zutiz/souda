...
export default function ShipmentsShow({ order, shipment }: { order: Record<string, unknown>; shipment: Record<string, unknown> }) {
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-4 lg:p-6">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2">
                        <p><strong>Status:</strong> <StatusBadge status={(shipment.status as string) ?? 'pending'} label={shipment.status as string} /></p>
                        <p><strong>Courier:</strong> {shipment.courier as string ?? '-'}</p>
                        <p><strong>Tracking:</strong> {shipment.tracking_number as string ?? '-'}</p>
                    </CardContent>
                </Card>
            </div>
        </AppLayout>
